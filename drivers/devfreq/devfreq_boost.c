...
static int lge_panel_notifier_cb(struct notifier_block *nb,
				unsigned long action, void *data)
{
	struct df_boost_drv *d = container_of(nb, typeof(*d), lge_panel_notif);
	int i;
	...
	for (i = 0; i < DEVFREQ_MAX; i++) {
		...
		switch (action) {
			...
			case LGE_PANEL_EVENT_POWER:
				if (panel_data->state == LGE_PANEL_POWER_VDDIO_ON) {
					clear_bit(SCREEN_OFF, &b->state);
					__devfreq_boost_kick_max(b,
						CONFIG_DEVFREQ_WAKE_BOOST_DURATION_MS);
				} else {
...
static int __init devfreq_boost_init(void)
{
	...
	for (i = 0; i < DEVFREQ_MAX; i++) {
		...
		thread[i] = kthread_run_perf_critical(cpu_prime_mask,
	...
	d->lge_panel_notif.notifier_call = lge_panel_notifier_cb;
	d->lge_panel_notif.priority = INT_MAX;
	ret = lge_panel_notifier_register_client(&d->lge_panel_notif);
	if (ret) {
